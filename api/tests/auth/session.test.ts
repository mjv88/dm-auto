...
import type { RunnerSession, AdminSession } from '../../src/middleware/session';
...
describe('createSessionToken / validateSessionToken', () => {
  it('round-trips a runner session', () => {
    ...
    expect(decoded.type).toBe('runner');
    expect((decoded as RunnerSession).runnerId).toBe(runnerSession.runnerId);
    expect((decoded as RunnerSession).pbxFqdn).toBe(runnerSession.pbxFqdn);
    expect(decoded.entraEmail).toBe(runnerSession.entraEmail);
  });
  ...
  it('round-trips an admin session', () => {
    ...
    expect((decoded as AdminSession).tid).toBe(adminSession.tid);
    expect((decoded as AdminSession).oid).toBe(adminSession.oid);
  });
